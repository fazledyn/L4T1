...
def grid_to_string(grid):
    return "".join([ chr(grid[row][col]) for col in range(4) for row in range(4) ])


@staticmethod
def print_list(arr):
    print(" ".join(hex(each) for each in arr))


@staticmethod
def print_grid(grid):
    print('\n'.join(' '.join((hex(x)) for x in row) for row in grid))
    print()
...
def xor_grid(first, second):
    ...
    for row in range(4):
        for col in range(4):
            new_grid[row][col] = first[row][col] ^ second[row][col]
    return new_grid
...
def sub_bytes(block):
    ...
    for row in range(4):
        for col in range(4):
            new_grid[row][col] = Sbox[block[row][col]]
    return new_grid
...
def inv_sub_bytes(block):
    ...
    for row in range(4):
        for col in range(4):
            new_grid[row][col] = InvSbox[block[row][col]]
    return new_grid
    ...
    for row in range(4):
        for col in range(4):
            for item in range(4):
                ...
                bv_out = Mixer[row][item].gf_multiply_modular(bv_val, AES_MOD, 8)
...
def inv_mix_columns(block):
    ...
    for row in range(4):
        for col in range(4):
            for item in range(4):
                bv_val = BitVector(intVal=block[item][col])
                bv_out = InvMixer[row][item].gf_multiply_modular(bv_val, AES_MOD, 8)
                new_grid[row][col] ^= bv_out.intValue()
    return new_grid
